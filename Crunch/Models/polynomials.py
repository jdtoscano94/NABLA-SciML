...
#     n_inputs: int =2
#     s: float = 10.0  # Standard deviation
#     def setup(self):
#         self.B = self.s * jax.random.normal(jax.random.PRNGKey(0), (self.features[0] // self.n_inputs, self.n_inputs)).T
#     @nn.compact
#     def __call__(self, X):
#         X_normalized = X
#         out = jnp.dot(X_normalized, self.B)
#         combined_out = jnp.hstack([
#             X_normalized,
#             jnp.sin(out),
#             jnp.cos(out)
#         ])
